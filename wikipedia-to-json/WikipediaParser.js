  ...
  fetchArticleElements: function (articleTitle) {
    return new Promise(function (resolve, reject) {
      articleTitle = encodeURIComponent(
        wikiUtils.optimizeArticleTitle(articleTitle)
      );
      console.log("[+] Sending requests to wikipedia API ...");
      const f =
        "https://ru.wikipedia.org/w/api.php?action=query&titles=" +
        encodeURI(articleTitle) +
        "&prop=pageimages&format=json&opithumbsize=500&redirects=";
      ...
      request(
        ...
        function (error, response, imageBody) {
          ...
          console.log(
            '[+] Downloading wikipedia article "' + articleTitle + '"...'
          );
          ...
          request(
            ...
            function (error, response, wikiBody) {
              if (error) {
                console.log(error);
              ...
              var elements = parseWikipediaHTML(
                wikiBody,
                articleTitle,
                imageURL
              );
...
function parseWikipediaHTML(html, articleTitle, imageURL) {
  ...
  var mainImageElement = imageURL
    ? createMainImageElement(imageURL, null, null)
    : extractMainImageFromInfobox(mainElement);
  ...
  for (var i = 0; i < wikiElements.length; i++) {
    ...
    switch (currentElement.tagName) {
      ...
      case "BLOCKQUOTE":
        ...
        var caption = null;
        var captionElements =
          currentElement.getElementsByClassName("templatequotecite");
      ...
      case "DL":
        ...
        if (listElement["list_items"].length > 0) elements.push(listElement);
        break;
      case "TABLE":
    ...
    if (
      currentElement.className == "thumb tright" ||
      currentElement.className == "thumb tleft"
    ) {
        ...
        if (subImageNodes[0].getElementsByTagName("img").length > 0) {
          ...
          var imageSide =
            currentElement.className == "thumb tright"
              ? IMAGE_SIDE_RIGHT
              : IMAGE_SIDE_LEFT;
          var imageElement = parseImage(
            subImageNodes[0].getElementsByTagName("img")[0],
            imageSide
          );
          if (
            ...
            imageElement = createMainImageElement(
              imageElement.url,
              imageElement.height,
              imageElement.width
            );
      ...
      if (wikiUtils.isValidLink(child.href)) {
        ...
        if (shouldMarkLinks) {
          parsedText +=
            ...
            child.textContent +
            linkSeperator +
            child.href.slice(child.href.lastIndexOf("/") + 1) +
            linkEndMark;
        } else {
          parsedText += child.textContent;
        }
...
function parseList(listElement) {
  ...
  switch (listElement.tagName) {
    ...
    case "OL":
      listType =
        listElement.tagName == "UL" ? LIST_TYPE_BULLETED : LIST_TYPE_NUMBERED;
...
function parseImage(imageElement, imageSide) {
  if (imageSide != IMAGE_SIDE_NONE && imageElement.tagName == "IMG") {
    ...
    var children =
      imageElement.parentNode.parentNode.getElementsByTagName("div");
    var caption = "";
...
function createQuoteElement(text, caption) {
  ...
  else return { type: TYPE_QUOTE, text: text };
}
...
function createMainImageElement(thumbUrl, height, width) {
  if (height != null && width != null)
    return {
      ...
      width: width,
      height: height,
      full_url: thumbImageToFullImageURL(thumbUrl),
    };
  else
    return {
      type: TYPE_MAIN_IMAGE_URL,
